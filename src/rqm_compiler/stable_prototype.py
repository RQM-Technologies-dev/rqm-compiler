...
def expectation_stable(circuit:Circuit, pauli:str|Iterable[str], *, max_terms:int=250_000, max_frontier_qubits:int=4, _plan_cache:dict|None=None)->StableReadoutResult:
 ...
 if labels=="Z"*circuit.num_qubits and direct.reason=="interleaved single-qubit gate":
  # Preserve the qualified unsafe-placement fallback and its Pauli-term cap.
  # A newly available frontier route must not silently bypass that contract.
  from .observables import expectation_pauli
  try:
   r=expectation_pauli(circuit,labels,max_terms=max_terms)
   return StableReadoutResult(r.value,"general_pauli_promoted",True,True,r.peak_terms,
    largest_intermediate=r.peak_terms,intermediate_unit="pauli_terms",query_promotion_count=1)
  except ObservableExpansionExceeded as exc:
   return StableReadoutResult(0j,"structured_exact",True,False,exc.peak_terms,str(exc),
    largest_intermediate=exc.peak_terms,intermediate_unit="pauli_terms",query_promotion_count=1)
